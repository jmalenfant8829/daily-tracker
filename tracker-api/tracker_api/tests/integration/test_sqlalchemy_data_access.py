...
from tracker_api.data_access.data_access import SQLAlchemyDataAccess, UserModel, DataAccessError
...
@pytest.fixture
def user():
    return User('Bob', 'secretpasswordhash', data_access)
...
def test_persist_work_times(db, work_times, data_access):
    ...
    user = User('Bob', 'secretpasswordhash', data_access)    
...
def test_update_work_times(db, work_times, data_access):
    """
    ...
    assert(DailyTaskTimeModel.query.filter_by(day=day2).first().minutes_spent == work_times[day2][task2.name]['minutes_spent'])
